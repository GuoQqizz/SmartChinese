...
function showQuestion(dom, data) {
	//console.log(data);
	if (data.SubjectType) {
		...
		for (var i = 0; i < arr.length; i++) {
			...
			if (i < arr.length - 1) {
				...
				for (var o = 0; o < data.StudentAnswer.length; o++) {
					//console.log(data.StudentAnswer[o]);
					if (data.StudentAnswer[o].Indx == i) {
						answer = data.StudentAnswer[o];
						break;
					}
				}
				if (answer) {
					html += '<span class="' + (answer.Scor == 1 ? "co479e93" : "cof47463") + '">(&nbsp;&nbsp;' + answer.Text +
						'&nbsp;&nbsp;)</span>';
				} else {
					html += '<span>(&nbsp;&nbsp;&nbsp;&nbsp;)</span>';
				}
	...
	function match(dom, data) {
		...
		for (var i = 0; i < array.length; i++) {
			...
			if (i % 2 == 0) {
				...
				if (data.RightOptionType) { //如果右边是图片
					html += d[0];
				} else {
					html += "<img src ='" + d[0] + "' />";
...
function drawLine(startObj, endObj, objdemo, anws) {
	...
	if (anws) {
		html = "<div class='rotate' style='background:#5ba597;top:" + y_start + "px;left:" + (x_start + 1) +
		...
		html = "<div class='rotate' style='background:#f47463;top:" + y_start + "px;left:" + (x_start + 1) +
			"px;width:" + length + "px;transform:rotate(" + deg + "deg)'>" +
			"<i class='arrow-img'></i>" +
			"<i class='con-img'></i></div>";
	}
