...
; (function ($) {
    $.fn.extend({
        ...
        initSelectOption: function (options) {
            ...
            options = $.extend(true, {
                'source': [],
                'selected': -1,//优先级 options.selecte>data-val
                'className': '',
            ...
            options.source.map(function (s) {
                let selected = options.selected == (s.key || s.id) ? "selected ='selected'" : '';
                innerHtml += `<option value="${s.key || s.id}"   class="${options.className}" ${selected}>${s.value || s.text}</option>`;
            })
            $($this).html(innerHtml);
            if (options.useSelect2 && $.fn.select2) {
                $($this).select2({ minimumResultsForSearch: Infinity, width: options.width });
            }
        },
