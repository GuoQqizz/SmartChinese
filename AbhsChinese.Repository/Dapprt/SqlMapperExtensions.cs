...
namespace AbhsChinese.Repository.Dapper.Contrib.Extensions
{
    /// <summary>
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            else
            {
                ...
                if (entityToInsert is System.Collections.IList)
                {
                    int count = (entityToInsert as System.Collections.IList).Count;
                    if (count > Consts.BulkInertCount && connection is SqlConnection)
                    {
                        if (transaction != null)
                        {
                            if (transaction is SqlTransaction)
                            {
                                return BulkInsert(connection as SqlConnection, transaction as SqlTransaction, entityToInsert, name, allProperties);
                            }
                        }
                        else
                        {
                            return BulkInsert(connection as SqlConnection, null, entityToInsert, name, allProperties);
                        }
                    }
                }
        ...
        private static int BulkInsert<T>(SqlConnection conn, SqlTransaction transaction, T entityToInsert, string tbName, List<PropertyInfo> allProperties)
        {
            ...
            var dt = GetTableSchema(allProperties, entityToInsert);
        ...
        private static DataTable GetTableSchema<T>(List<PropertyInfo> allProperties, T entityToInsert)
        {
            var list = entityToInsert as System.Collections.IList;
            int count = list.Count;
            DataTable dt = new DataTable();
            allProperties.ForEach(ap =>
            {
                dt.Columns.Add(new DataColumn(ap.Name, ap.PropertyType));
            });
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                allProperties.ForEach(ap =>
                {
                    var val = type.GetProperty(ap.Name).GetValue(obj, null);
                    dr[ap.Name] = val;
                });
                dt.Rows.Add(dr);
            }
        ...
        public static bool UpdateByAudit<T>(this IDbConnection connection, T entityToUpdate, List<string> fields, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            ...
            {
                nonIdProps = nonIdProps.Where(x => fields.Contains(x.Name)).ToList();
            }
